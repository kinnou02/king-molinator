...
KBMNTIGPAF_Settings = nil
chKBMNTIGPAF_Settings = nil
...
AF.Anrak = {
	...
	Settings = {
		...
		AlertsRef = {
			...
			Storm = KBM.Defaults.AlertObj.Create("blue")
		},
		TimersRef = {
			Enabled = true,
...
AF.Lang.Ability.Spines:SetFrench("Épine de Terre")
...
function AF:InitVars()
	...
	KBMNTIGPAF_Settings = self.Settings
	chKBMNTIGPAF_Settings = self.Settings
end
...
function AF:SwapSettings(bool)
	...
	if bool then
		KBMNTIGPAF_Settings = self.Settings
		self.Settings = chKBMNTIGPAF_Settings
	else
		chKBMNTIGPAF_Settings = self.Settings
		self.Settings = KBMNTIGPAF_Settings
	end
...
function AF:LoadVars()	
	if KBM.Options.Character then
		KBM.LoadTable(chKBMNTIGPAF_Settings, self.Settings)
	else
		KBM.LoadTable(KBMNTIGPAF_Settings, self.Settings)
	end
...
function AF:Start()
	...
	self.Anrak.AlertsRef.Storm = KBM.Alert:Create(self.Lang.Ability.Storm[KBM.Lang], nil, true, true, "blue")
