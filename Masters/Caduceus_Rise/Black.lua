...
KBMMMCRCBS_Settings = nil
chKBMMMCRCBS_Settings = nil
-- Link Mods
...
function MOD:InitVars()
	...
	KBMMMCRCBS_Settings = self.Settings
	chKBMMMCRCBS_Settings = self.Settings
...
function MOD:SwapSettings(bool)
	...
	if bool then
		KBMMMCRCBS_Settings = self.Settings
		self.Settings = chKBMMMCRCBS_Settings
	else
		chKBMMMCRCBS_Settings = self.Settings
		self.Settings = KBMMMCRCBS_Settings
	end
...
function MOD:LoadVars()	
	if KBM.Options.Character then
		KBM.LoadTable(chKBMMMCRCBS_Settings, self.Settings)
	else
		KBM.LoadTable(KBMMMCRCBS_Settings, self.Settings)
	end
...
function MOD:UnitHPCheck(unitDetails, unitID)	
	if unitDetails and unitID then
		if not unitDetails.player then
			if unitDetails.name == self.Black.Name then
...
function MOD:DefineMenu()
	self.Menu = Instance.Menu:CreateEncounter(self.Black, self.Enabled)
end
