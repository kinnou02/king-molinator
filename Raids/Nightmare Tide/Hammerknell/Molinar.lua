...
KBMNTKM_Settings = nil
chKBMNTKM_Settings = nil
...
function KM:InitVars()
	...
	KBMNTKM_Settings = self.Settings
	chKBMNTKM_Settings = self.Settings
...
function KM:SwapSettings(bool)
	if bool then
		KBMNTKM_Settings = self.Settings
		self.Settings = chKBMNTKM_Settings
	else
		chKBMNTKM_Settings = self.Settings
		self.Settings = KBMNTKM_Settings
	end
end

function KM:LoadVars()
	if KBM.Options.Character then
		KBM.LoadTable(chKBMNTKM_Settings, KM.Settings)
	else
		KBM.LoadTable(KBMNTKM_Settings, KM.Settings)
	end
	
	if KBM.Options.Character then
		chKBMNTKM_Settings = KM.Settings
	else
		KBMNTKM_Settings = KM.Settings
	end
...
function KM:Start()
	...
	self.PercentageMon = KBM.PercentageMon:Create(self.King, self.Prince, 4)
